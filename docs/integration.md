...
Warichu reflows after document, font, writing-mode or effective inline-size changes. Equivalent observer notifications leave the editable DOM stable. When a native pointer selection has changed in the DOM but is still waiting for ProseMirror synchronization, presentation transactions are deferred so reflow cannot restore the preceding selection. Body paragraphs retain native mouse selection; custom glyph hits and dragging apply only when starting inside annotation text. Keep ProseMirror’s trailing separator and break visible so a native caret can reach the end of a paragraph containing an inline atom such as Ruby.

The editor wraps native Ruby in an internal `.mdi-ruby-atom` node view to keep trailing caret hits outside its annotation. This wrapper is not part of the schema, clipboard HTML or saved MDI; the Ruby itself remains selectable.
